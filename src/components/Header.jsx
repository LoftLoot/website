...
const QuickFilterChips = ({ onCommit, onReset, selectedCollection, selectedDecade, collections = [], decades = [], availableCollections = [], availableDecades = [], hasMounted }) => {
    ...
    useEffect(() => {
        ...
        if (el) {
            ...
            checkScroll();
            setTimeout(() => { checkScroll(); setGradientsReady(true); }, 50); 
        }
        return () => {
    ...
    const startScrolling = (direction) => {
        ...
        const animate = () => {
            if (scrollRef.current) { scrollRef.current.scrollLeft += scrollSpeed.current; }
    ...
    const handleChipClick = (e, kind, value) => {
        ...
        if (kind === 'Collection' && selectedCollection === value) finalValue = 'All';
        else if (kind === 'Era' && selectedDecade === value) finalValue = 'All';
        onCommit({ type: 'filter', payload: { kind, value: finalValue } });
    };

    return (
        <div className="relative w-full border-b border-[#514d46]/5 bg-[#fffbf0]">
            <div className={`absolute left-0 top-0 bottom-0 w-12 z-10 pointer-events-none ${gradientsReady ? 'transition-opacity duration-300' : ''} ${showLeft ? 'opacity-100' : 'opacity-0'}`} style={{ background: 'linear-gradient(to right, #fffbf0 20%, transparent)' }} />
            <div className={`hidden lg:block absolute left-0 top-0 bottom-0 w-12 z-20 cursor-w-resize ${showLeft ? 'block' : 'hidden'}`} onMouseEnter={() => showLeft && startScrolling('left')} onMouseLeave={stopScrolling} />
            <div className={`absolute right-0 top-0 bottom-0 w-12 z-10 pointer-events-none ${gradientsReady ? 'transition-opacity duration-300' : ''} ${showRight ? 'opacity-100' : 'opacity-0'}`} style={{ background: 'linear-gradient(to left, #fffbf0 20%, transparent)' }} />
            <div className={`hidden lg:block absolute right-0 top-0 bottom-0 w-12 z-20 cursor-e-resize ${showRight ? 'block' : 'hidden'}`} onMouseEnter={() => showRight && startScrolling('right')} onMouseLeave={stopScrolling} />
            <div ref={scrollRef} className="flex w-full overflow-x-auto no-scrollbar items-center gap-2 px-4 pb-3 pt-1 select-none">
                {chips.map((chip, idx) => {
                    if (chip === "|") return <div key={`sep-${idx}`} className="h-5 w-px bg-[#514d46]/20 mx-1 shrink-0"></div>;
                    let isActive = false, isAvailable = true, color = '#487ec8', kind = 'Collection';
                    if (decades.includes(chip)) {
                        kind = 'Era'; isActive = selectedDecade === chip; isAvailable = availableDecades.includes(chip); color = FILTER_COLORS.Era;
                    } else {
                        kind = 'Collection'; isActive = selectedCollection === chip; isAvailable = availableCollections.includes(chip); color = FILTER_COLORS.Collection;
                    }
                    return (
                        <button key={chip} disabled={!isAvailable} onClick={(e) => handleChipClick(e, kind, chip)} style={{ backgroundColor: isActive ? color : (isAvailable ? 'white' : '#f4f4f5'), borderColor: isActive ? color : (isAvailable ? '#E0E8F0' : 'transparent'), color: isActive ? 'white' : (isAvailable ? '#514d46' : 'rgba(81, 77, 70, 0.3)') }} className={`whitespace-nowrap px-3.5 py-1.5 rounded-full text-sm font-bold border ${hasMounted ? 'transition-all' : ''} shadow-sm ${isActive ? "shadow-md" : isAvailable ? "" : "cursor-default"}`}>{chip}</button>
                    );
                })}
            </div>
        </div>
        ...
        <div ref={containerRef} className="relative w-full h-full group z-[60]">
            <div className="flex w-full h-full">
                <div className="relative flex-1">
                    <input ref={inputRef} type="text" placeholder="Search our loft..." value={search} onChange={(e) => onSearchUpdate(e.target.value)} onKeyDown={handleKeyDown} onFocus={() => { if(search && search.length >= 2) { const hasResults = (suggestions?.filters?.length > 0 || suggestions?.products?.length > 0); if (hasResults) setIsOpen(true); onSearchUpdate(search); } }} className={`w-full h-full bg-white text-[#514d46] py-2.5 pl-4 pr-10 font-bold placeholder:text-[#514d46]/40 focus:outline-none rounded-l-2xl border-2 border-r-0 border-[#514d46]/20 focus:border-[#d35153] focus:border-r-0 ${hasMounted ? 'transition-colors' : ''}`} />
                    {search && (<button onClick={() => { onSearchUpdate(''); onCommit({ type: 'query', value: '' }); inputRef.current?.blur(); }} className="absolute right-2 top-1/2 -translate-y-1/2 bg-[#514d46]/20 text-white rounded-full p-1 hover:bg-[#514d46]/40 transition-colors"><X size={14} strokeWidth={3} /></button>)}
                </div>
                <button onClick={() => { onCommit({ type: 'query', value: search }); setIsOpen(false); }} className={`bg-[#d35153] text-white px-5 flex items-center justify-center hover:bg-[#b94547] cursor-pointer rounded-r-2xl border-2 border-[#d35153] border-l-0 ${hasMounted ? 'transition-colors' : ''}`}><Search className="w-5 h-5" strokeWidth={3} /></button>
            </div>
            {isOpen && suggestions && (
                        ...
                        <div className="flex flex-col border-b border-[#514d46]/10">
                            ...
                            {suggestions.filters.map((filter, idx) => (
                                <div key={`filter-${idx}`} onClick={() => handleCommit('filter', filter)} onMouseEnter={() => setActiveIndex(idx)} className={`px-4 py-3 flex items-center justify-between cursor-pointer transition-colors ${activeIndex === idx ? 'bg-[#f4f4f5]' : ''}`}>
                                    <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-[#d35153] border border-[#514d46]/10 shadow-sm"><Filter size={14} strokeWidth={3} /></div><span className="font-bold text-[#514d46] text-base"><HighlightedText text={filter.label} highlight={search} /></span></div><span className="text-xs font-bold text-[#514d46]/60 bg-white px-2 py-1 rounded-md border border-[#514d46]/10 shadow-sm">{filter.kind}</span>
                                </div>
                            ))}
                        </div>
                    )}
                            ...
                            {suggestions.products.map((prod, idx) => {
                                ...
                                return (
                                    <div key={prod.id} onClick={() => handleCommit('product', prod)} onMouseEnter={() => setActiveIndex(globalIdx)} className={`px-4 py-3 flex items-center gap-4 cursor-pointer transition-colors ${activeIndex === globalIdx ? 'bg-[#f4f4f5]' : ''}`}>
                                        <div className="w-12 h-12 shrink-0 rounded-lg overflow-hidden bg-white relative border border-[#514d46]/10 shadow-sm"><img src={prod.mainImage} alt="" className="w-full h-full object-cover" /></div>
                                        <div className="flex flex-col min-w-0"><span className="font-bold text-[#514d46] text-base truncate"><HighlightedText text={prod.name} highlight={search} /></span>{prod.snippet ? (<span className="text-sm text-[#514d46]/70 italic truncate"><HighlightedText text={prod.snippet.text} highlight={search} /></span>) : (<span className="text-sm text-[#514d46]/60 truncate">{prod.manufacturer} • {prod.releaseDate || 'Unknown'}</span>)}</div>
                                        <div className="ml-auto flex items-center gap-2">{prod.stock < 1 && <span className="text-[10px] font-bold bg-white text-[#d35153] px-2 py-0.5 rounded-full uppercase tracking-wider border border border-[#d35153]/10 shadow-sm">Sold</span>}<ChevronRight size={16} className="text-[#514d46]/50" /></div>
                                    </div>
                                );
                            })}
                        </div>
                    )}
                    {hasMore && (<div onClick={() => { onCommit({ type: 'query', value: search }); setIsOpen(false); }} onMouseEnter={() => setActiveIndex(filterCount + productCount)} className={`p-3 text-center border-t border-[#514d46]/10 cursor-pointer ${activeIndex === filterCount + productCount ? 'bg-[#f4f4f5]' : ''}`}><span className="text-sm font-bold text-[#d35153] flex items-center justify-center gap-1">View {suggestions.totalProducts - suggestions.products.length} more results <ArrowRight size={12} strokeWidth={3} /></span></div>)}
                </div>
            )}
        </div>
    );
...
const Header = React.memo(({ currentView, isProductView, onCatalogueClick, onAboutClick, onHomeClick, search, onSearchUpdate, onCommit, suggestions, selectedCollection, selectedDecade, collections, decades, availableCollections, availableDecades, enableSticky = true }) => {
  ...
  useIsomorphicLayoutEffect(() => {
      ...
      const updateDimensions = () => {
          ...
          if (container) maxScrollRef.current = Math.max(0, container.scrollHeight - container.clientHeight);
      };
      updateDimensions();
      ...
      return () => { obs.disconnect(); window.removeEventListener('resize', updateDimensions); };
  }, [isShopMain]); 

  useEffect(() => {
      if (!isShopMain || !enableSticky) {
          setIsSticky(false);
          if (stickyRef.current) { stickyRef.current.style.opacity = ''; stickyRef.current.style.transform = ''; }
          return;
      }
      let rafId;
      const loop = () => {
          ...
          if (shouldBeSticky !== isStickyRef.current) {
              ...
              if (shouldBeSticky) {
                  if (stickyRef.current) {
                      stickyRef.current.style.position = 'fixed'; stickyRef.current.style.top = '0'; stickyRef.current.style.left = '0'; stickyRef.current.style.right = '0';
                      stickyRef.current.classList.add('shadow-md'); stickyRef.current.style.transform = `translateY(${maxHide}px)`; stickyRef.current.style.opacity = '0';
                  }
                  if (spacerRef.current) spacerRef.current.style.display = 'block'; 
                  currentTranslate.current = maxHide;
              } else {
                  if (stickyRef.current) {
                      stickyRef.current.style.position = 'relative'; stickyRef.current.style.top = 'auto'; stickyRef.current.style.left = 'auto'; stickyRef.current.style.right = 'auto';
                      stickyRef.current.classList.remove('shadow-md'); stickyRef.current.style.transform = ''; stickyRef.current.style.opacity = '';
                  }
          ...
          if (isStickyRef.current && stickyRef.current) {
              ...
              let mvOpacity = 1 - ratio; mvOpacity = Math.max(0, Math.min(1, mvOpacity));
  ...
  useIsomorphicLayoutEffect(() => {
      ...
      calculateRepeats();
      const onResize = () => { cancelAnimationFrame(rafId); rafId = requestAnimationFrame(calculateRepeats); };
      ...
      <div className="hidden lg:block bg-[#fffbf0] shadow-sm z-[60] relative">
        <div className="mx-auto px-4 h-[86px] flex items-center justify-between">
            ...
            <div className="flex justify-end items-center gap-6 shrink-0 relative z-[61]">
                <button onClick={currentView !== 'shop' ? onCatalogueClick : undefined} disabled={currentView === 'shop' && !isProductView} className={`text-lg font-bold ${hasMounted ? 'transition-colors' : ''} ${currentView === 'shop' && !isProductView ? 'text-[#487ec8] cursor-default' : 'text-[#514d46]/60 hover:text-[#487ec8] active:scale-95'}`}>Catalogue</button>
                <div className="h-6 w-0.5 bg-[#514d46]/10"></div>
                <button onClick={currentView !== 'about' ? onAboutClick : undefined} disabled={currentView === 'about'} className={`text-lg font-bold ${hasMounted ? 'transition-colors' : ''} ${currentView === 'about' ? 'text-[#487ec8] cursor-default' : 'text-[#514d46]/60 hover:text-[#487ec8] active:scale-95'}`}>About Us</button>
            </div>
        </div>
      </div>

      <div className="lg:hidden relative z-[60]">
          {/* FIX: Force mobile header to stay within viewport */}
          ...
          <div ref={stickyRef} className={`relative ${!isShopMain ? 'pb-1' : 'pb-0'} bg-[#fffbf0] z-50 will-change-transform ${(isSticky && enableSticky && isShopMain) ? 'shadow-md' : ''}`} style={(isSticky && enableSticky && isShopMain) ? { position: 'fixed', top: 0, left: 0, right: 0 } : {}}>
              <div className="px-4 pb-2 pt-4"><SearchInput search={search} onSearchUpdate={onSearchUpdate} onCommit={onCommit} suggestions={suggestions} /></div>
              {isShopMain && (<QuickFilterChips onCommit={onCommit} onReset={onHomeClick} selectedCollection={selectedCollection} selectedDecade={selectedDecade} collections={collections} decades={decades} availableCollections={availableCollections} availableDecades={availableDecades} hasMounted={hasMounted} />)}
          </div>
      </div>
    </>
  );
